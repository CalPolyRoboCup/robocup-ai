...
class orbit_ball(move_to):
  ...
  def run(self):
    ...
    if np.linalg.norm(speed_mod_vec) < self.speed_mod_distance:
      move_target = move_target + speed_mod_vec * abs(rotation_angle) * self.speed_mod_factor
    ...
    if np.linalg.norm(move_target - self.robot.loc) > 300:
