...
class get_open(action):
  ...
  def get_better_loc(self):
    ...
    if self.robot.is_blue:
      target_point = self.robot.loc*np.array([0,-1]) + np.array([1000, 0])
    else:
      target_point = self.robot.loc*np.array([0,-1]) + np.array([-1000, 0])
    for i in range(self.samples):
      sample_point = np.random.uniform(-1, 1, size = [2])*np.array([2000, 3000]) + target_point
...
if __name__ == "__main__":
  ...
  ttime = clock.tick()
  ind = 2
  for i in game.yellow_robots[1:4]:
    ...
    if ind == max_bots_per_team:
      ind = 1
  ind = 1
  for i in game.yellow_robots[4:5]:
    i.task = 1
    i.add_action(get_open([game.yellow_robots[0].loc, game.yellow_robots[1].loc, game.yellow_robots[2].loc, game.yellow_robots[3].loc], [1,.7, .7, .7], game.blue_robots, game.yellow_robots))
    ind += 1
  while 1:
    ind = 1
    while ind != max_bots_per_team - 1:
      ...
      if ind >= 4 and ind < 5:
        game.yellow_robots[ind].action.points[1] = game.yellow_robots[1].loc
        game.yellow_robots[ind].action.points[2] = game.yellow_robots[2].loc
        game.yellow_robots[ind].action.points[3] = game.yellow_robots[3].loc
      ind += 1
