...
class strategy:
  ...
  def offensive_strategy(self, exclude = []):
    ...
    if not self.attacking:
      ...
      for fa in free_allies[:-2]:
        fa.add_action(get_open([self.game.ball.controler.loc, self.their_goal], self.enemies))
        
      #two "fielders" set up double play shots
      i = 0
      for fa in free_allies[-2:]:
        fa.add_action(get_open([self.game.ball.controler.loc, free_allies[i]], self.enemies))
        i += 1
  ...
  def neutral_strategy(self):
    ...
    for i in range(len(self.allies)):
      if i == closest:
        continue
      self.allies[i].add_action(cover_ball(self.my_goal, self.game.ball, min_interpose_offset = 1000))
  ...
  def update(self):
    ...
    if not type(self.goalie.action) is cover:
      self.goalie.add_action(cover_ball(self.my_goal, self.game.ball, interpose_factor = 1, min_interpose_offset = 1000))
